...
from interbase.ibase import (
    ...
    isc_dpb_num_buffers, isc_dpb_password,
    isc_dpb_sql_dialect, isc_dpb_sql_role_name,
    isc_dpb_user_name, isc_dpb_version1,
    ...
    isc_info_base_level, isc_info_bpage_errors, 
    isc_info_current_memory, isc_info_db_id,
    ...
    isc_info_ipage_errors, isc_info_limbo, isc_info_marks,
    isc_info_max_memory, 
    ...
    isc_info_set_page_buffers, isc_info_sql_stmt_delete,
    isc_info_sql_stmt_exec_procedure,
    isc_info_sql_stmt_insert, isc_info_sql_stmt_select,
    isc_info_sql_stmt_update, isc_info_sweep_interval,
    isc_info_update_count, isc_info_user_names, isc_info_version,
    isc_info_writes, isc_tpb_concurrency, isc_tpb_consistency,
    isc_tpb_exclusive, isc_tpb_lock_read,
    isc_tpb_no_rec_version, isc_tpb_nowait, isc_tpb_protected,
    isc_tpb_read, isc_tpb_read_committed, isc_tpb_rec_version,
    isc_tpb_shared, isc_info_tpage_errors,
    isc_tpb_version3, isc_tpb_wait, isc_tpb_write,
    isc_tpb_lock_write, isc_dpb_password_enc, isc_dpb_sys_encrypt_password,

    b, ord2, int2byte,
    mychr, mybytes, myunicode,
    mylong, StringType, IntType,
    LongType, FloatType, ListType,
    UnicodeType, TupleType, xrange,
    charset_map,
    ...
    ISC_EVENT_CALLBACK, ISC_ARRAY_DESC, ISC_ARRAY_DESC_V2,
    blr_varying, blr_varying2, blr_text,
    blr_text2, blr_short, blr_long,
    blr_int64, blr_float, blr_d_float,
    blr_double, blr_timestamp, blr_sql_date,
    blr_sql_time, ibclient_API,
    SQLDA_version2, isc_segment, isc_db_handle,
    isc_tr_handle, isc_stmt_handle, isc_blob_handle
)
...
def load_api(ib_library_name=None, embedded=False):
    ...
    if not hasattr(sys.modules[__name__],'api'):
        setattr(sys.modules[__name__],'api',ibclient_API(ib_library_name, embedded or embedded_env_var))
...
def build_dpb(user, password, sql_dialect, role, charset, buffers, force_write,
    ...
    if enc_password:
        addString(isc_dpb_password_enc, enc_password)
    if sep_db_password:
        addString(isc_dpb_sys_encrypt_password, sep_db_password)
    return b('').join(params)


def connect(
        dsn='', user=None, password=None, host=None, database=None,
        sql_dialect=3, role=None, charset=None, buffers=None,
        force_write=None, no_reserve=None, db_key_scope=None,
        isolation_level=ISOLATION_LEVEL_READ_COMMITED, encrypted_password=None,
        connection_class=None, ib_library_name=None, sep_password=None,
        ssl=False,
    ...
    :param string server_public_path: Server public file path.
    :param bool embedded: Use embedded client library.
    ...
    if not dsn:
        dsn = host if host else ''
        if dsn.endswith(':'):
            raise ProgrammingError(
                f"Host must not end with a colon. You should specify host='{host[:-1]}' rather than host='{host}'."
            )

        if ssl:
            dsn += "?ssl=true"
            if server_public_file:
                dsn += "?serverPublicFile=%s" % server_public_file
            if client_cert_file:
                dsn += "?clientCertFile=%s" % client_cert_file
            if client_pass_phrase:
                dsn += "?clientPassPhrase=%s" % client_pass_phrase
            if client_pass_phrase_file:
                dsn += "?clientPassPhraseFile=%s" % client_pass_phrase_file
            if server_public_path:
                dsn += "?serverPublicPath=%s" % server_public_path
            dsn += "??"
    ...
    dpb = build_dpb(
        ...
        db_key_scope=db_key_scope,
        enc_password=encrypted_password,
        sep_db_password=sep_password
    )
    ...
    if db_api_error(_isc_status):
        raise exception_from_status(DatabaseError, _isc_status, "Error while connecting to database:")
...
def create_database(sql='', sql_dialect=3, dsn='', user=None, password=None,
                    ...
                    connection_class=None, ib_library_name=None,
                    ssl=False,
                    client_pass_phrase_file=None,
                    client_pass_phrase=None,
                    client_cert_file=None,
    ...
    if sql:
        ...
        sql = "CREATE DATABASE '%s' USER '%s' PASSWORD '%s' WITH ADMIN OPTION" % (dsn, user, password)
        if page_size:
...
class Connection(object):
    ...
    def database_info(self, info_code, result_type):
        ...
        if request_buffer[0] != res_buf[0]:
            raise InternalError("Result code does not match request code: %i." % res_buf[0])
    ...
    def transaction_info(self, info_code, result_type):
        ...
        Thin wrapper around InterBase API `isc_transaction_info` call.
    ...
    def create_savepoint(self, name):
        ...
        """
        self.__check_attached()
        return self._main_transaction.create_savepoint(name)

    def release_savepoint(self, name):
        self.__check_attached()
        return self._main_transaction.release_savepoint(name)
    ...
    #: For example it's 'LI-V6.3.2.26540 InterBase 2.5' for InterBase 2.5.2
...
class ConnectionWithSchema(Connection):
    ...
    .. note::
       ...
       Use `connection_class` parameter of :func:`connect` or :func:`create_database`
...
class EventBlock(object):
    ...
    .. warning: Internally used class not intended for direct use.
    """
...
class PreparedStatement(object):
 ...
 which has range [%s,%s].""" % (str(value),
    ...
    def __XSQLDA2Tuple(self, xsqlda):
        ...
        for sqlvar in xsqlda.sqlvar[:xsqlda.sqld]:
            ...
            if ((sqlvar.sqltype & 1) != 0) and (bool(sqlvar.sqlind)
                                                and sqlvar.sqlind.contents.value < 0):
                value = None
                ...
                if sqlvar.sqlsubtype in (ibase.CHARSET_UTF_8, None):
                    reallength = sqlvar.sqllen // 4
                elif sqlvar.sqlsubtype == ibase.CHARSET_UNICODE_FSS:
                    reallength = sqlvar.sqllen // 3
                elif sqlvar.sqlsubtype in (
                    ibase.CHARSET_BIG_5,
                    ibase.CHARSET_EUCJ_0208,
                    ibase.CHARSET_GB_2312,
                    ibase.CHARSET_KSC_5601,
                    ibase.CHARSET_SJIS_0208,
                    ibase.CHARSET_UNICODE_BE,
                    ibase.CHARSET_UNICODE_LE
                ):
                    reallength = sqlvar.sqllen // 2
                else:
                ...
                arrayid = ISC_QUAD(bytes_to_uint(val[:4]),
                                        bytes_to_uint(val[4:sqlvar.sqllen]))
                arraydesc = ISC_ARRAY_DESC_V2(0)
                sqlsubtype = self.cursor._connection._get_array_sqlsubtype(sqlvar.relname,
                                                                           sqlvar.sqlname)
                api.isc_array_lookup_bounds2(self._isc_status,
                                              self.cursor._connection._db_handle,
                                              self.cursor._transaction._tr_handle,
                                              sqlvar.relname,
                                              sqlvar.sqlname,
                ...
                if db_api_error(self._isc_status):
                    raise exception_from_status(DatabaseError,
                                                self._isc_status,
                                                "Cursor.read_otput_array/isc_array_lookup_bounds2:")
                value_type = arraydesc.array_desc_dtype
                value_scale = int.from_bytes(arraydesc.array_desc_scale, byteorder="big")
                ...
                api.isc_array_get_slice2(self._isc_status,
                                          self.cursor._connection._db_handle,
                                          self.cursor._transaction._tr_handle,
                                          arrayid, arraydesc,
                                          value_buffer, tsize)
                if db_api_error(self._isc_status):
                    raise exception_from_status(DatabaseError,
                                                self._isc_status,
                                                "Cursor.read_otput_array/isc_array_get_slice2:")
            ...
            # Change view. If subscription is active, all changed records
            # will be returned with the appropriate sqlind variable
            if ((sqlvar.sqltype & 1) != 0) and (bool(sqlvar.sqlind)
                                                and sqlvar.sqlind.contents.value >= 0):
                sqlind = sqlvar.sqlind.contents.value & ~SQLIND_NULL
                if sqlind & SQLIND_UPDATE or sqlind & SQLIND_INSERT or sqlind & SQLIND_DELETE:
                    value = [value, sqlind]
        ...
        if dim == len(dimensions)-1:
            for i in xrange(dimensions[dim]):
                if dtype in (blr_text,blr_text2):
                    ...
                    # CHAR with multibyte encoding requires special handling
                    if subtype in (ibase.CHARSET_UTF_8,
                                   None):
                        reallength = esize // 4
                    elif subtype == ibase.CHARSET_UNICODE_FSS:
                        reallength = esize // 3
                    elif subtype in (ibase.CHARSET_BIG_5,
                                   ibase.CHARSET_EUCJ_0208,
                                   ibase.CHARSET_GB_2312,
                                   ibase.CHARSET_KSC_5601,
                                   ibase.CHARSET_SJIS_0208,
                    ...
                    raise OperationalError("Unsupported InterBase ARRAY subtype: %i" % dtype)
                value.append(val)
                bufpos += esize
        else:
            for i in xrange(dimensions[dim]):
        ...
        for i in xrange(xsqlda.sqld):
            ...
            if value == None:
                ...
                if (sqlvar.sqltype & 1) == 0:
                    # NULLs were not allowed, so set it allowed or IB will complain
                    sqlvar.sqltype += 1
                ...
                if ((vartype != SQL_BLOB and
                    ...
                    if db_api_error(self._isc_status):
                        raise exception_from_status(DatabaseError,
                                                    self._isc_status,
                                                    "Cursor.write_otput_array/isc_array_lookup_bounds2:")
                    value_type = arraydesc.array_desc_dtype
                    value_scale = int.from_bytes(arraydesc.array_desc_scale, byteorder="big")
                    value_size = arraydesc.array_desc_length
                    if value_type in (blr_varying,blr_varying2):
                    ...
                    api.isc_array_put_slice2(self._isc_status,
                                              self.cursor._connection._db_handle,
                                              self.cursor._transaction._tr_handle,
                    ...
                    if db_api_error(self._isc_status):
                        raise exception_from_status(DatabaseError,
                                                    self._isc_status,
                                                    "Cursor.read_otput_array/isc_array_put_slice2:")
    ...
    #: but wasn't dropped, and could be still used for another execution.
    closed = property(__get_closed)
...
class Cursor(object):
    ...
    def callproc(self, procname, parameters=None):
        ...
        :raises ProgrammingError: When more parameters than expected are supplied.
    ...
    def fetchonemap(self):
        ...
        :returns: :class:`ibcore._RowMapping` of returned values, or None when
    ...
    def fetchmanymap(self, size=None):
        ...
        :param integer size: Max. number of rows to fetch.
        :returns: List of :class:`ibcore._RowMapping` instances, one such instance for
                  each row.
        :raises DatabaseError: When error is returned by server.
        :raises ProgrammingError: When underlying :class:`PreparedStatement` is
                                  closed, statement was not yet executed, or
    ...
    def itermap(self):
        ...
        :returns: Iterator that yields :class:`ibcore._RowMapping` instance
                  like :meth:`fetchonemap`.
        """
        return utils.Iterator(self.fetchonemap, None)
    def setinputsizes(self, sizes):
        """Required by Python DB API 2.0, but pointless for InterBase, so it
        does nothing."""
        pass
...
class Transaction(object):
    ...
    def rollback(self, retaining=False, savepoint=None):
        ...
        if retaining and savepoint:
            raise ProgrammingError("Can't rollback to savepoint while"
                                   " retaining context")
        if savepoint:
            api.isc_rollback_savepoint(
                self._isc_status,
                self._tr_handle,
                ctypes.c_char_p(savepoint.encode(self.__python_charset)),
                0
            )
        else:
            if retaining:
    ...
    def create_savepoint(self, name):
        ...
        """
        api.isc_start_savepoint(
            self._isc_status,
            self._tr_handle,
            ctypes.c_char_p(name.encode(self.__python_charset)),
        )
        if db_api_error(self._isc_status):
    ...
    def release_savepoint(self, name):
        """
        Description: Releasing a savepoint destroys savepoint named by the identifier without
        affecting any work that has been performed subsequent to its creation.

        :param string name: savepoint_name
        """
        api.isc_release_savepoint(
            self._isc_status,
            self._tr_handle,
            ctypes.c_char_p(name.encode(self.__python_charset)),
        )
    ...
    def trans_info(self, request):
        ...
        for infoCode in request:
            ...
            # versions of IB expose constant isc_info_tra_isolation:
...
class ConnectionGroup(object):
    ...
    def create_savepoint(self, name):
        ...
        return self._transaction.create_savepoint(name)
    ...
    def release_savepoint(self, name):
        ...
        """
        self.__require_non_empty_group('savepoint')
        return self._transaction.release_savepoint(name)
